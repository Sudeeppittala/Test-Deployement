...
const PLACEHOLDER_ATTENDEES: Attendee[] = [
  {
    ...
    quote: "Attendee testimonial or experience quote will appear here once added to the Google Sheet.",
    profile_pic_url: "",
    linkedin_url: "",
    verified: "TRUE",
    visible: "TRUE"
...
export function useAttendeeData() {
  const [attendees, setAttendees] = useState<Attendee[]>(PLACEHOLDER_ATTENDEES);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<any>(null);

  useEffect(() => {
    const fetchAttendees = async () => {
      const sheetUrl = (import.meta as any).env.VITE_ATTENDEES_SHEET_URL;

      if (!sheetUrl) {
        // Silently use placeholders if env variable is not set
        setAttendees(PLACEHOLDER_ATTENDEES.filter(a => a.visible === "TRUE"));
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const visibleList = parsed
          .map((item: any) => ({
            ...
            verified: String(item.verified || 'FALSE').toUpperCase(),
            visible: String(item.visible || 'FALSE').toUpperCase()
          }))
          .filter((item: any) => item.visible === "TRUE");

        setAttendees(visibleList);
      } catch (err) {
        console.error("Failed to load attendees from sheet, falling back to placeholders:", err);
        setError(err);
        setAttendees(PLACEHOLDER_ATTENDEES.filter(a => a.visible === "TRUE"));
      } finally {
        setLoading(false);
