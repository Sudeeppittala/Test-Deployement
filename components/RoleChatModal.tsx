...
const RoleChatModal: React.FC<RoleChatModalProps> = ({ role, category, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
        
        {/* Header */}
        <div className="bg-slate-50 dark:bg-gray-800 p-8 border-b border-gray-100 dark:border-gray-700 flex justify-between items-start">
          <div>
             <div className="inline-block px-3 py-1 bg-primary/10 dark:bg-primary-light/10 text-primary dark:text-primary-light text-xs font-black uppercase tracking-widest rounded-full mb-3">
               {category}
             </div>
             <h3 className="font-sans font-bold tracking-tight text-4xl text-slate-900 dark:text-white">{role}</h3>
          </div>
          <button 
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-400 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-gray-500 flex items-center justify-center transition-all"
          >
        ...
        <div className="p-8 overflow-y-auto">
          ...
          <div className="mb-10">
            <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-4">What you'll do</h4>
          ...
          <div className="grid md:grid-cols-2 gap-10 mb-10">
             <div>
                <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-4">Key Skillset</h4>
                <div className="flex flex-wrap gap-2">
                  {data.skills.map((skill, i) => (
                    <span key={i} className="px-3 py-1.5 bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 rounded-lg text-sm font-bold border border-slate-200 dark:border-gray-700">
                      {skill}
                    </span>
                  ))}
                </div>
             ...
             <div className="space-y-6">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-2">Market Demand</h4>
                  <div className="flex items-center gap-2">
                    <div className={`h-3 w-3 rounded-full ${data.demand === 'Very High' || data.demand === 'Explosive' ? 'bg-green-500' : 'bg-primary dark:bg-primary-light'}`}></div>
                    <span className="text-slate-900 dark:text-white font-sans font-bold text-xl tracking-tight">{data.demand}</span>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-2">Avg. Salary (Entry-Mid)</h4>
                  <span className="text-slate-900 dark:text-white font-sans font-bold text-xl tracking-tight">{data.salary}</span>
                </div>
             </div>
          </div>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-gray-800 flex justify-end">
