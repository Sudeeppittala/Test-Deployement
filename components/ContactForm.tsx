    ...
    <section className="py-32 px-6 bg-white" ref={ref} id="main-contact-form">
      <div className="max-w-4xl mx-auto">
        ...
        <ScrollReveal animation="scale-up" delay={0.2}>
          <div className="bg-slate-50 p-10 md:p-16 rounded-[2rem] border border-gray-100 relative overflow-hidden shadow-sm">
            
            {/* Success Overlay - Animated */}
            <div 
              className={`absolute inset-0 bg-slate-50 flex flex-col items-center justify-center z-20 text-center p-8 transition-all duration-500 ease-out-expo ${
                status === 'success' ? 'opacity-100 visible scale-100' : 'opacity-0 invisible scale-95'
              }`}
            >
                 <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center text-green-600 mb-6 animate-pulse-slow">
            ...
            <form onSubmit={handleSubmit} className={`space-y-10 transition-all duration-500 ${status === 'submitting' ? 'opacity-50 scale-[0.98] pointer-events-none grayscale' : ''}`}>
                ...
                {Object.values(Audience).map((type) => (
                  <button
                    ...
                    className={`font-sans px-8 py-3 rounded-full text-sm font-bold uppercase tracking-wider transition-all duration-300 ease-out-expo active:scale-95 ${
                      ...
                      : 'bg-white text-gray-400 hover:text-primary border border-gray-200 hover:border-primary/30'
                    }`}
                  >
                    {type}
                  </button>
              ...
              <div className="grid md:grid-cols-2 gap-8 relative">
                ...
                <div className="space-y-2">
                  ...
                  <input required name="name" type="text" placeholder="Jane Doe" className="font-sans w-full px-5 py-4 rounded-xl bg-white border border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium" />
                </div>
                <div className="space-y-2">
                  <label className="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Email</label>
                  <input required name="email" type="email" placeholder="jane@example.com" className="font-sans w-full px-5 py-4 rounded-xl bg-white border border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium" />
                </div>

                <div className="space-y-2">
                  <label className="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Mobile Number</label>
                  <input required name="phone" type="tel" placeholder="+91 98765 43210" className="font-sans w-full px-5 py-4 rounded-xl bg-white border border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium" />
                </div>
                ...
                <div className="space-y-2">
                   ...
                   <input required name="organization" type="text" placeholder="Start typing..." className="font-sans w-full px-5 py-4 rounded-xl bg-white border border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium" />
                </div>
              </div>
